...
export const CollageCreator: React.FC<CollageCreatorProps> = ({ theme, setTheme, onGenerate, resultUrl, isProcessing, selectedCount, enhancedTheme }) => {
  return (
    <div className="bg-gray-800 rounded-2xl p-6 space-y-4">
      <div>
        <h3 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-500 mb-2">Artistic Collage Creator</h3>
        <p className="text-sm text-gray-400">Select 2 or more images from the queue above, describe a theme, and let AI build a creative collage.</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div className="md:col-span-2">
            <label htmlFor="collage-theme" className="block text-sm font-medium mb-2">Collage Theme</label>
            <textarea
              id="collage-theme"
              value={theme}
              ...
              placeholder="e.g., 'A surreal, dreamlike landscape with pastel colors'"
      ...
      {(!isProcessing && (resultUrl || enhancedTheme)) && (
        <div className="mt-4 space-y-4">
          <h4 className="text-lg font-semibold">Collage Result</h4>
          {resultUrl && (
          ...
          {enhancedTheme && (
            <div className="text-left bg-gray-900/50 p-3 rounded-lg">
              <h4 className="text-sm font-semibold text-indigo-400 mb-1">Enhanced Prompt Used:</h4>
